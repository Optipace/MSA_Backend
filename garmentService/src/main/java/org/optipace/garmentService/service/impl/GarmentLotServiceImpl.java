...
public class GarmentLotServiceImpl implements GarmentLotService {
	...
	public SingleResponse<PageResponse<ListOfGarmentLotResponse>> getAllGarmentLots(Pageable pageable) {
		...
		Pageable sortedPageable = pageable.getSort().isSorted() ? pageable
				: PageRequest.of(pageable.getPageNumber(), pageable.getPageSize(),
						Sort.by(Sort.Order.asc("lotNumber").nullsLast()));

		Page<GarmentLot> garmentLotPage = garmentLotRepository.findByRecordStatus('A', sortedPageable);

		Page<ListOfGarmentLotResponse> responsePage = garmentLotPage
				.map(garmentLot -> modelMapper.map(garmentLot, ListOfGarmentLotResponse.class));
	...
	public SingleResponse<?> createGarmentLot(GarmentLotRequest request, String userId) {
		...
		return SingleResponse.success(HttpStatus.SC_CREATED,
				"Garment lot successfully created with lot number: " + garmentLot.getLotNumber());
	}
